...
def _map_light_from_cw(light: cwm.LightInstance) -> Light:
    return Light(
...
def _map_light_to_cw(light: Light) -> cwm.LightInstance:
...
def from_cw_extension(extension: cwm.Extension) -> Extension:
    base_kwargs = {
    ...
    match extension:
        ...
        case cwm.ExtensionLightEffect():
            return ExtensionLightEffect(
                instances=[_map_light_from_cw(li) for li in extension.instances],
                **base_kwargs,
            )
...
def to_cw_extension(extension: Extension) -> cwm.Extension:
    match extension:
        case ExtensionDoor():
            e = cwm.ExtensionDoor()
        ...
        case ExtensionLightEffect():
            ...
            e.instances = [_map_light_to_cw(li) for li in extension.instances]
        case _:
...
def from_cw_entity(entity: cwm.Entity | cwm.EntityMloInstance) -> Entity:
    if isinstance(entity, cwm.EntityMloInstance):
        cls = EntityMloInstance
        extra_kwargs = {
            "group_id": entity.group_id,
            "floor_id": entity.floor_id,
            "default_entity_sets": [jenkhash.try_resolve_maybe_hashed_name(item.value) for item in entity.default_entity_sets],
    ...
    return cls(
        ...
        extensions=[ext for e in entity.extensions if (ext := from_cw_extension(e)) is not None],
        **extra_kwargs,
    )


def to_cw_entity(entity: Entity | EntityMloInstance) -> cwm.Entity:
    if isinstance(entity, EntityMloInstance):
        e = cwm.EntityMloInstance()
        e.group_id = entity.group_id
        e.floor_id = entity.floor_id
        e.default_entity_sets = [
            cwm.DefaultEntitySetsList.Item(tag_name="Item", value=name) for name in entity.default_entity_sets
        ]
    ...
    e.extensions = [to_cw_extension(ext) for ext in entity.extensions]
    return e
...
def load_map_types_from_cw(t: cw.CMapTypes) -> AssetMapTypes:
    ...
    def _map_entity_set(entity_set: cw.EntitySet) -> MloEntitySet:
        return MloEntitySet(
            ...
            entities=[from_cw_entity(e) for e in entity_set.entities],
        )
    ...
    def _map_archetype(archetype: cw.BaseArchetype) -> Archetype:
        ...
        match arch_type:
            ...
            case ArchetypeType.MLO:
                extra_kwargs = {
                    ...
                    "entities": [from_cw_entity(e) for e in archetype.entities],
        ...
        return Archetype(
            ...
            extensions=[ext for e in archetype.extensions if (ext := from_cw_extension(e)) is not None],
            **extra_kwargs,
        )

    name = jenkhash.try_resolve_maybe_hashed_name(t.name)
    archetypes = [_map_archetype(a) for a in t.archetypes]
...
def save_map_types_to_cw(asset: AssetMapTypes) -> cw.CMapTypes:
    ...
    def _map_entity_set(entity_set: MloEntitySet) -> cw.EntitySet:
        ...
        s.entities = [to_cw_entity(e) for e in entity_set.entities]
        return s
    ...
    def _map_archetype(archetype: Archetype) -> cw.BaseArchetype:
        ...
        a.extensions = [to_cw_extension(e) for e in archetype.extensions]
        if archetype.type == ArchetypeType.TIME:
            a.time_flags = archetype.time_flags
        elif archetype.type == ArchetypeType.MLO:
            a.mlo_flags = archetype.mlo_flags
            a.entities = [to_cw_entity(e) for e in archetype.entities]
    ...
    t = cw.CMapTypes()
    t.name = asset.name
    t.archetypes = [_map_archetype(a) for a in asset.archetypes]
    return t
