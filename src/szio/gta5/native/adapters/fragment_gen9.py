...
from ...shattermaps import compress_shattermap
from ._utils import (
    from_native_mat34,
    to_native_sphere,
    to_native_uv,
...
from .fragment import _get_vehicle_windows, _to_mat34

# --- Standalone load/save functions ---


def load_fragment(f: pmg9.Fragment) -> AssetFragment:
    """Convert a native gen9 Fragment to an AssetFragment dataclass."""
    from .bound import load_bound
    from .cloth import from_native_bridge, from_native_verlet_cloth_edge
    from .drawable import _map_light_from_native
    from .drawable_gen9 import load_frag_drawable
    ...
    def _load_child(c: pmg9.FragmentTypeChild, idx: int, lod: pmg9.FragmentPhysicsLod) -> PhysChild:
        return PhysChild(
            bone_tag=c.bone_id,
    ...
    def _load_lod(lod: pmg9.FragmentPhysicsLod) -> PhysLod:
    ...
    def _load_glass_window(g: pmg9.BGPaneModelInfoBase) -> FragGlassWindow:
    ...
    def _load_verlet_cloth(c: pm.VerletCloth):
    ...
    def _load_controller(c: pm.ClothController):
    ...
    def _load_cloth(c: pmg9.FragmentEnvCloth) -> EnvCloth:
        return EnvCloth(
...
def save_fragment_to_native(asset: AssetFragment) -> pmg9.Fragment:
    """Convert an AssetFragment dataclass to a native gen9 Fragment."""
    from .bound import save_bound_to_native
    from .cloth import to_native_bridge, to_native_morph_controller, to_native_verlet_cloth_edge
    from .drawable import _map_light_to_native
    from .drawable_gen9 import save_frag_drawable_to_native
    ...
    if asset.physics is not None:
        ...
        def _save_child(child: PhysChild) -> pmg9.FragmentTypeChild:
            c = pmg9.FragmentTypeChild()
        ...
        def _link_group_indices(groups, children) -> int:
        ...
        def _link_child_collisions(children, collisions, damaged_collisions):
        ...
        g = pmg9.FragmentPhysicsLodGroup()
        g.high_lod = l
        f.physics_lod_group = g

    # Save glass windows
    def _save_glass_window(gw: FragGlassWindow) -> pmg9.BGPaneModelInfoBase:
        C = pmg9.FvfChannel
        F = pmg9.BufferFormat
        fvf = pmg9.Fvf()
    ...
    def _save_cloth(cloth: EnvCloth) -> pmg9.FragmentEnvCloth:
        c = pmg9.FragmentEnvCloth()
